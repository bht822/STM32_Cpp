...
int main()
{
    ...
    while(1) {
        ...
        if(status) {
					...
					mcal::reg::reg_access<std::uint32_t,std::uint32_t,mcal::reg::gpioa_odr,(1U<<5)>::reg_or();

        }
        else {
            mcal::reg::reg_access<std::uint32_t,std::uint32_t,mcal::reg::gpioa_odr,(1U<<5)>::reg_not();
        }
        pseudoDelayMs(10);
    }
}
